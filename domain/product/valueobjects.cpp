...
float BBox::getXCenter() {
    return (this->xtl + this->width / 2.0f);
}

float BBox::getYCenter() {
...
Product::Product(const BBox& _bbox, const QString& _className, const Tag& _tag)
    : bbox(_bbox), tag(_tag), className(_className) {}

Product::Product(const Product& _other)
    : bbox(_other.bbox), tag(_other.tag), className(_other.className) {}
